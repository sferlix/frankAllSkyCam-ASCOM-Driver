...
- **Telegram notifications**: an alert is sent when conditions transition safe → unsafe (and back), and texting `now` or `status` to the bot replies with a screenshot of the live status window plus (optionally) a copy of the camera's live image.
...
| `Notifications` | Telegram bot token, chat ID, AllSkyCam image URL, enabled flag — also editable from "Notification settings" |
